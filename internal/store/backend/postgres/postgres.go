...
func Bootstrap(db *sql.DB) error {
	statements := []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_pg_skills_review_state_active ON skills(review_state, is_active)`,
		fmt.Sprintf(`CREATE INDEX IF NOT EXISTS idx_pg_skills_search_vector ON skills USING GIN ((%s))`, skillSearchVectorExpression()),
		`ALTER TABLE skills ADD COLUMN IF NOT EXISTS review_state TEXT NOT NULL DEFAULT 'approved'`,
		`ALTER TABLE skills ADD COLUMN IF NOT EXISTS created_by TEXT NOT NULL DEFAULT 'system'`,
		`ALTER TABLE skills ADD COLUMN IF NOT EXISTS reviewed_by TEXT NOT NULL DEFAULT 'system'`,
		`ALTER TABLE skills ADD COLUMN IF NOT EXISTS reviewed_at TEXT`,
		`ALTER TABLE skills ADD COLUMN IF NOT EXISTS review_notes TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE skills ALTER COLUMN review_state SET DEFAULT 'approved'`,
		`ALTER TABLE skills ALTER COLUMN created_by SET DEFAULT 'system'`,
		`ALTER TABLE skills ALTER COLUMN reviewed_by SET DEFAULT 'system'`,
		`ALTER TABLE skills ALTER COLUMN review_notes SET DEFAULT ''`,
