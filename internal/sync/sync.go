...
var (
	...
	storeGetSynced      = func(s store.Contract) (map[string]bool, error) { return s.GetSyncedChunks() }
	storeExportData     = func(s store.Contract) (*store.ExportData, error) { return s.Export() }
	storeImportData     = func(s store.Contract, d *store.ExportData) (*store.ImportResult, error) { return s.Import(d) }
	storeRecordSynced   = func(s store.Contract, chunkID string) error { return s.RecordSyncedChunk(chunkID) }
)
...
func New(s store.Contract, syncDir string) *Syncer {
	return &Syncer{
		store:     s,
		syncDir:   syncDir,
...
func NewLocal(s store.Contract, syncDir string) *Syncer {
...
func NewWithTransport(s store.Contract, transport Transport) *Syncer {
	return &Syncer{
		store:     s,
		transport: transport,
	}
