...
// Lore's own MCP tools — don't count these as "tool calls" for session stats
...
async function isLoreRunning(): Promise<boolean> {
  try {
...
/**
 * Strip <private>...</private> tags before sending to lore.
...
export const Lore: Plugin = async (ctx) => {
  ...
  // Track which sessions we've already ensured exist in lore
  ...
  /**
   * Ensure a session exists in lore. Idempotent — calls POST /sessions
  ...
  const running = await isLoreRunning()
  if (!running) {
    try {
  ...
  if (oldProject !== project) {
    await loreFetch("/projects/migrate", {
      method: "POST",
      body: { old_project: oldProject, new_project: project },
    })
  }

  // Auto-import: if .lore/manifest.json exists in the project repo,
  // run `lore sync --import` to load any new chunks into the local DB.
  ...
  try {
    const manifestFile = `${ctx.directory}/.lore/manifest.json`
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (input.tool === "Task" && output && sessionId) {
        ...
        if (text.length > 50) {
          await loreFetch("/observations/passive", {
            method: "POST",
            body: {
              session_id: sessionId,
    ...
    // This ensures the agent ALWAYS knows about Lore, even after compaction.
    //
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      // compacted summary to Lore. The new agent reads the compacted
