	...
	return []docsContract{
		{
			...
			MustContain: []string{
				...
				"`LORE_BOOTSTRAP_ADMIN_EMAIL` and `LORE_BOOTSTRAP_ADMIN_PASSWORD` are also required in staging",
		...
		},
		{
			...
			MustContain: []string{
				...
				"HTTP `/mcp` requires `Authorization: Bearer <jwt>`.",
		...
		},
		{
			...
			MustContain: []string{
				...
				"`LORE_BOOTSTRAP_ADMIN_PASSWORD` — REQUIRED.",
				"Authorization: Bearer <jwt>",
				"`/health` and `/mcp` smoke check",
		...
		},
		{
			...
			MustContain: []string{
				...
				"Lore may default the bootstrap email locally, but it never invents a password.",
			},
			MustNotContain: commonForbidden,
		},
		{
